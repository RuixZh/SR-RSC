...
class Attention(nn.Module):
    ...
    def forward(self, emb):
        ...
        alphas = F.softmax(emb_att, 0) # (rel_size, batch_size, 1)
        emb_sumamry = (emb * alphas).sum(0)
...
class SemanticAttention(nn.Module):
    ...
    Simple GAT layer, similar to https://arxiv.org/abs/1710.10903
    """
    def __init__(self, in_ft, out_ft, nb_rel):
        super().__init__()
        self.in_ft = in_ft
        self.out_ft = out_ft
        self.nb_rel = nb_rel
    ...
    def forward(self, x):
        ...
        #h_prime=(PN)*F'
        semantic_attentions = torch.mm(h_prime, torch.t(self.q)).view(self.nb_rel, -1)
...
#         print(semantic_attentions)
        semantic_attentions = semantic_attentions.view(self.nb_rel, 1,1)
        semantic_attentions = semantic_attentions.repeat(1, N, self.in_ft)
#        print(semantic_attentions)

        #input_embedding = P*N*F
        input_embedding = x.view(self.nb_rel,N,self.in_ft)
